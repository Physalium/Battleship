...
public static class ShootingAI
{
   public static Position Fire(Player firingPlayer, Player firedUponPlayer)
   {
      ...
      if (seekingShot != null) return seekingShot;
   ...
   private static Position? FireSeekingShot(Player firedUponPlayer)
   {
      ...
      List<Tile> damagedShipTiles = firedUponPlayer.Board.Tiles.Where(t => t.IsHit && t.Ship != null && !t.Ship.IsSunk).ToList();
      foreach (Tile? tile in damagedShipTiles)
      {
         List<Tile>? fireCandidates = firedUponPlayer.Board.GetNotHitNeighbors(tile.Coordinates);
         if (fireCandidates.Count == 0) continue;
