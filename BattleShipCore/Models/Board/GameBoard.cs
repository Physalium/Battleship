...
public class GameBoard
{
   ...
   public List<Tile> GetNeighbors(Position coordinates)
   {
      ...
      if (column >= 1) panels.Add(Tiles.At(row, column - 1));
      if (row >= 1) panels.Add(Tiles.At(row - 1, column));
      if (row < Constants.BOARD_SIZE - 1) panels.Add(Tiles.At(row + 1, column));
      if (column < Constants.BOARD_SIZE - 1) panels.Add(Tiles.At(row, column + 1));
      return panels;
   }
